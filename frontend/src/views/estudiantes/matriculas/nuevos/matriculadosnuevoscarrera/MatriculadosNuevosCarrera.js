...
const MatriculadosSexoFacultad = () => {
    ...
    const fetchData = async (gestion) => {
        try {
            const url = gestion
                ? `${config.API_URL}/matriculados-nuevos-carrera?gestion=${gestion}`
                : `${config.API_URL}/matriculados-nuevos-carrera`;
    ...
    const exportToExcel = () => {
        ...
        XLSX.writeFile(workbook, `Matriculados_Sexo_Carrera_Nuevos_${selectedGestion}.xlsx`);
    };
