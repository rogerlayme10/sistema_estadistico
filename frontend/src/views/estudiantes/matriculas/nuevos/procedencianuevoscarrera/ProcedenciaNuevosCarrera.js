...
const ProcedenciaNuevosCarrera = () => {
    const [procedencias, setProcedencias] = useState([]);
    const [gestiones, setGestiones] = useState([]);
    const [selectedGestion, setSelectedGestion] = useState(2023);

    const fetchData = useCallback((gestion) => {
        fetch(`${config.API_URL}/matriculados-nuevos-procedencia-carrera?gestion=${gestion}`)
            .then((response) => {
                if (!response.ok) {
    ...
    return (
        <Row>
            <Col>
                <Card>
                    <CardHeader>
                        Poblacion estudiantil Matriculados Nuevos por Procedencia Educativa, según  Carrera.
                    ...
                    <Table bordered>
                        <thead>
                            <tr>
                                ...
                                <th colSpan="5" className="text-center">Procedencia Educativa</th>
...
export default ProcedenciaNuevosCarrera;
