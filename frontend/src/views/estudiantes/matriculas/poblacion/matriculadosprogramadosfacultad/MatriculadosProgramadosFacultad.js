...
const MatriculadosprogramadosFacultad = () => {
    ...
    const fetchData = async (gestion) => {
        try {
            const url = gestion
                ? `${config.API_URL}/matriculados-programados-facultad?gestion=${gestion}`
                : `${config.API_URL}/matriculados-programados-facultad`;
    ...
    const downloadChartImage = () => {
        ...
        if (chart) {
            ...
            link.href = imageURL;
            link.download = `Grafico_Matriculados_Sexo_Faculta_Nuevos${selectedGestion}.png`;
            link.click();
        }
    };
    ...
    const chartData = {
        ...
        datasets: [
            {
                label: 'Masculino',
                data: data.map(item => item.total_semestre1),
                backgroundColor: 'rgba(75, 192, 192, 0.6)',
            },
            {
                label: 'Femenino',
                data: data.map(item => item.total_semestre2),
