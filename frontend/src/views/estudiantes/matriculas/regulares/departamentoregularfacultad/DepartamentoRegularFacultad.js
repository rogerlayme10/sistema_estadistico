...
const DepartamentoFacultad = () => {
    ...
    const fetchData = async (year) => {
        const response = await fetch(`${config.API_URL}/facultad-regular-departamento/${year}`);
        const result = await response.json();
        setData(result);
    };

    const fetchYears = async () => {
        const response = await fetch(`${config.API_URL}/facultad-regular-departamento`);
        const result = await response.json();
        setYears(result);
    ...
    return (
        <Row>
            <Col xs={12}>
                <Card>
                    <CardHeader>
                        Población Estudiantil Regular Matriculados por Lugar de Nacimiento y Departamento, Según Facultad.
                        <Form.Select
                            value={selectedYear}
