...
const PoblacionProcedenciaEducativa = () => {
    const [procedencias, setProcedencias] = useState([]);
    const [gestiones, setGestiones] = useState([]);
    const [selectedGestion, setSelectedGestion] = useState(2023);

    const fetchData = useCallback((gestion) => {
        fetch(`${config.API_URL}/matriculados-regular-procedencia?gestion=${gestion}`)
            .then((response) => {
                if (!response.ok) {
    ...
    return (
        <Row>
            <Col>
                <Card>
                    <CardHeader>
                    Población Estudinatil Regular por Procedencia Educativa, Según Facultad.
...
export default PoblacionProcedenciaEducativa;
