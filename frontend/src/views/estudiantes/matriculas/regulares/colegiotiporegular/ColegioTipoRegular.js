...
const ColegioTipoRegular = () => {
    ...
    const fetchData = async (year) => {
        const response = await fetch(`${config.API_URL}/matriculas-regular?gestion=${year}`);
        const result = await response.json();
        setData(result);
    };

    const fetchYears = async () => {
        const response = await fetch(`${config.API_URL}/matriculas-regular?gestiones=true`);
        const result = await response.json();
        setYears(result);
    ...
    return (
        <Row>
            <Col xs={12} md={6} xl={6}>
                <Card>
                    <CardHeader>
                        Población Regular Estudiantil Matrigulada por Sexo, según Tipo de Colegio.
                        <Form.Select
                            value={selectedYear}
...
export default ColegioTipoRegular;
