...
const MatriculadosSexoFacultad = () => {
    ...
    const fetchData = async (gestion) => {
        try {
            const url = gestion
                ? `${config.API_URL}/matriculados-regular-facultad?gestion=${gestion}`
                : `${config.API_URL}/matriculados-regular-facultad`;
    ...
    return (
        <Row>
            <Col xs={12} md={6} xl={6}>
                <Card>
                    <CardHeader>
                        Población Regular Estudiantil Matrigulada por Sexo, según Facultad.
                        <Form.Select
