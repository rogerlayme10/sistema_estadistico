...
const RegularProcedenciaCarrera = () => {
    const [procedencias, setProcedencias] = useState([]);
    const [gestiones, setGestiones] = useState([]);
    const [selectedGestion, setSelectedGestion] = useState(2023);

    const fetchData = useCallback((gestion) => {
        fetch(`${config.API_URL}/matriculados-regular-procedencia-carrera?gestion=${gestion}`)
            .then((response) => {
                if (!response.ok) {
    ...
    return (
        <Row>
            <Col>
                <Card>
                    <CardHeader>
                        Poblacion estudiantil Regular por Procedencia Educativa, según Carrera.
...
export default RegularProcedenciaCarrera;
