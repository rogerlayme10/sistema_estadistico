...
const DepartamentoCarrera = () => {
    ...
    const fetchData = async (year) => {
        const response = await fetch(`${config.API_URL}/carrera-regular-departamento/${year}`);
    ...
    const fetchYears = async () => {
        const response = await fetch(`${config.API_URL}/carrera-regular-departamento`);
        const result = await response.json();
        setYears(result);
    ...
    return (
        <Row>
            <Col xs={12}>
                <Card>
                    <CardHeader>
                    Población Estudiantil Regular Matriculados por Lugar de Nacimiento y Departamento, Según Carrera.
                        <Form.Select
                            value={selectedYear}
