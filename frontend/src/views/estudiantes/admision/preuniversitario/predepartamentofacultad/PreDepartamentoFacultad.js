...
const PreDepartamentoFacultad = () => {
   ...
   //const chartRef = useRef(null);
    ...
    const fetchData = async (year) => {
        const response = await fetch(`${config.API_URL}/pre-departamento-facultad/${year}`);
        const result = await response.json();
        setData(result);
    };

    const fetchYears = async () => {
        const response = await fetch(`${config.API_URL}/pre-departamento-facultad`);
        const result = await response.json();
        setYears(result);
    ...
    return (
        <Row>
            <Col xs={12}>
                <Card>
                    <CardHeader>
                        Cursos PRE Universitarios por Lugar de Nacimiento y Departamento, según Facultad.
                        <Form.Select
                            value={selectedYear}
...
export default PreDepartamentoFacultad;
