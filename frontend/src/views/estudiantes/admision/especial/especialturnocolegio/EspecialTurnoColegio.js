...
const EspecialTurnoColegio = () => {
    ...
    const fetchData = async (year) => {
        const response = await fetch(`${config.API_URL}/especial-turnocol?gestion=${year}`);
        const result = await response.json();
        setData(result);
    };

    const fetchYears = async () => {
        const response = await fetch(`${config.API_URL}/especial-turnocol?gestiones=true`);
        const result = await response.json();
        setYears(result);
    ...
    return (
        <Row>
            <Col xs={12} md={6} xl={6}>
                <Card>
                    <CardHeader>
                        Admision Estudiantil por Admision Especial Turno Colegio.
                        <Form.Select
                            value={selectedYear}
...
export default EspecialTurnoColegio;
