...
const EspecialDepartamentoFacultad = () => {
    ...
    const fetchData = async (year) => {
        const response = await fetch(`${config.API_URL}/especial-departamento-facultad/${year}`);
        const result = await response.json();
        setData(result);
    };

    const fetchYears = async () => {
        const response = await fetch(`${config.API_URL}/especial-departamento-facultad`);
        const result = await response.json();
        setYears(result);
    ...
    return (
        <Row>
            <Col xs={12}>
                <Card>
                    <CardHeader>
                        Población Universitaria por Facultad según Departamento de Nacimiento Admisiones Especiales.
                        <Form.Select
                            value={selectedYear}
...
export default EspecialDepartamentoFacultad;
