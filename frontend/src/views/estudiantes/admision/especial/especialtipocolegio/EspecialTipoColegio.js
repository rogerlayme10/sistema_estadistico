...
const EspecialTipoColegio = () => {
    ...
    const fetchData = async (year) => {
        const response = await fetch(`${config.API_URL}/especial-tipocol?gestion=${year}`);
        const result = await response.json();
        setData(result);
    };

    const fetchYears = async () => {
        const response = await fetch(`${config.API_URL}/especial-tipocol?gestiones=true`);
        const result = await response.json();
        setYears(result);
...
export default EspecialTipoColegio;
