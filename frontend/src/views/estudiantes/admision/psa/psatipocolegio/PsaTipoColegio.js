...
const PsaTipoColegio = () => {
    ...
    const fetchData = async (year) => {
        const response = await fetch(`${config.API_URL}/psa-tipocol?gestion=${year}`);
        const result = await response.json();
        setData(result);
    };

    const fetchYears = async () => {
        const response = await fetch(`${config.API_URL}/psa-tipocol?gestiones=true`);
        const result = await response.json();
        setYears(result);
    ...
    return (
        <Row>
            <Col xs={12} md={6} xl={6}>
                <Card>
                    <CardHeader>
                        Admision Estudiantil por Psa Tipo Colegio.
                        <Form.Select
                            value={selectedYear}
