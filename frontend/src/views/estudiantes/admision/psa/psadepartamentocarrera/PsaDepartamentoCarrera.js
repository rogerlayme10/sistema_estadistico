...
const PsaDepartamentoCarrera = () => {
    ...
    const fetchData = async (year) => {
        const response = await fetch(`${config.API_URL}/psa-departamento-carrera/${year}`);
        const result = await response.json();
        setData(result);
    };

    const fetchYears = async () => {
        const response = await fetch(`${config.API_URL}/psa-departamento-carrera`);
        const result = await response.json();
        setYears(result);
    ...
    return (
        <Row>
            <Col xs={12}>
                <Card>
                    <CardHeader>
                        P.S.A. por Lugar de Nacimiento y Departamento,según Carrera.
                        <Form.Select
                            value={selectedYear}
...
export default PsaDepartamentoCarrera;
