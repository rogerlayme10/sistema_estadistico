...
const AdmisionPrincipal = () => {
    const [chartData, setChartData] = useState({});
    const chartRef = useRef(null);

    useEffect(() => {
        fetch(`${config.API_URL}/admision-principal`)
            .then(response => response.json())
            .then(data => {
                const gestionLabels = data.map(item => item.gestion);
                const totalPostulantes = data.map(item => item.total_postulantes);
                const totalAprobados = data.map(item => item.total_aprobados); // Nueva línea para total_aprobados
                ...
                setChartData({
                    ...
                    datasets: [
                        ...
                        {
                            label: 'Aprobados', // Nueva línea para aprobados
                            data: totalAprobados,
...
export default AdmisionPrincipal;
