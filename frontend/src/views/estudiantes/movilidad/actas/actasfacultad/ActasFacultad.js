...
const ActasFacultad = () => {
  ...
  const fetchData = async (year = null) => {
    try {
      const url = year ? `${config.API_URL}/actas-facultad/${year}` : `${config.API_URL}/actas-facultad`;
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `certificado_ActasFacultad${gestion}.xlsx`);
  };
  ...
  const downloadChartImage = () => {
    ...
    saveAs(base64Image, `Certificado_ActasFacultad${gestion}.png`); // Guardar imagen como archivo PNG
  };
