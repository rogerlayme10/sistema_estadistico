...
const ReadmisionFacultad = () => {
  ...
  const fetchData = async (year = null) => {
    try {
      const url = year ? `${config.API_URL}/readmision-facultad/${year}` : `${config.API_URL}/readmision-facultad`;
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `Readmision_Facultad${gestion}.xlsx`);
  };
  ...
  const downloadChartImage = () => {
    ...
    saveAs(base64Image, `Readmision_Facultad${gestion}.png`); // Guardar imagen como archivo PNG
  };
...
export default ReadmisionFacultad;
