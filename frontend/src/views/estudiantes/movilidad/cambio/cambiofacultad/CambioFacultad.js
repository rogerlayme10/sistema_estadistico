...
const CambioFacultad = () => {
  ...
  const fetchData = async (year = null) => {
    try {
      const url = year ? `${config.API_URL}/cambio-facultad/${year}` : `${config.API_URL}/cambio-facultad`;
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `Cambio_Facultad${gestion}.xlsx`);
  };
  ...
  const downloadChartImage = () => {
    ...
    saveAs(base64Image, `Cambio_Facultad${gestion}.png`); // Guardar imagen como archivo PNG
  };
  ...
  return (
    <Row>
      <Col xs={12} md={5} xl={5}>
        <Card>
          <CardHeader>
            Cambio de carrera, facultad segun sexo
            <Form.Select
              className="mt-2"
              value={gestion}
