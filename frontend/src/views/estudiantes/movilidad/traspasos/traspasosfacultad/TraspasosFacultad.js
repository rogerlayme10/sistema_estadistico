...
const TraspasosFacultad = () => {
  ...
  const fetchData = async (year = null) => {
    try {
      const url = year ? `${config.API_URL}/traspasos-facultad/${year}` : `${config.API_URL}/traspasos-facultad`;
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `Traspaso_Facultad${gestion}.xlsx`);
  };
  ...
  const downloadChartImage = () => {
    ...
    saveAs(base64Image, `Traspaso_Facultad${gestion}.png`); // Guardar imagen como archivo PNG
  };
  ...
  return (
    <Row>
      <Col xs={12} md={5} xl={5}>
        <Card>
          <CardHeader>
            Traspasos por Sexo, según Facultad.
            <Form.Select
              className="mt-2"
              value={gestion}
...
export default TraspasosFacultad;
