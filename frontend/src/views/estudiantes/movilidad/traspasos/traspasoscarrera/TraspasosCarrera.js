...
const TraspasosCarrera = () => {
    ...
    const fetchData = async (year = null) => {
        try {
            const url = year ? `${config.API_URL}/traspasos-carrera/${year}` : `${config.API_URL}/traspasos-carrera`;
    ...
    const exportToExcel = () => {
        ...
        XLSX.writeFile(wb, `Traspaso_Carrera${gestion}.xlsx`);
    };
    ...
    return (
        <Row>
            <Col >
                <Card>
                    <CardHeader>
                        Traspasos por sexo, según Carrera.
                        <Form.Select 
                            className="mt-2" 
                            value={gestion} 
...
export default TraspasosCarrera;