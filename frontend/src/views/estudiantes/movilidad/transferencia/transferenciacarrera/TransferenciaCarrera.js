...
const TransferenciaCarrera = () => {
    ...
    const fetchData = async (year = null) => {
        try {
            const url = year ? `${config.API_URL}/transferencia-carrera/${year}` : `${config.API_URL}/transferencia-carrera`;
    ...
    const barData = {
        labels: sortedData.map(row => row.carrera),  // Etiquetas de cada facultad
        datasets: [
            /*{
              label: 'Masculino',
    ...
    const exportToExcel = () => {
        ...
        XLSX.writeFile(wb, `Transferencias_Carrera${gestion}.xlsx`);
    };
    ...
    const downloadChartImage = () => {
        ...
        saveAs(base64Image, `Transferencias_Carrera${gestion}.png`); // Guardar imagen como archivo PNG
    };
...
export default TransferenciaCarrera;