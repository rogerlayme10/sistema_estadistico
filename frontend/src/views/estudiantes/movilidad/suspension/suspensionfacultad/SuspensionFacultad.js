...
const SuspensionFacultad = () => {
  ...
  const fetchData = async (year = null) => {
    try {
      const url = year ? `${config.API_URL}/suspension-facultad/${year}` : `${config.API_URL}/suspension-facultad`;
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `Suspension_Facultad${gestion}.xlsx`);
  };
  ...
  const downloadChartImage = () => {
    ...
    saveAs(base64Image, `Suspension_Facultad${gestion}.png`); // Guardar imagen como archivo PNG
  };
  ...
  return (
    <Row>
      <Col xs={12} md={5} xl={5}>
        <Card>
          <CardHeader>
            Suspension por Sexo, según Facultad.
            <Form.Select
              className="mt-2"
              value={gestion}
...
export default SuspensionFacultad;
