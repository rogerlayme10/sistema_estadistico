...
const ActasFacultad = () => {
  ...
  const fetchData = async (year = null) => {
    try {
      const url = year ? `${config.API_URL}/notas-facultad/${year}` : `${config.API_URL}/notas-facultad`;
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `Certificado_NotasFacultad${gestion}.xlsx`);
  };
  ...
  const downloadChartImage = () => {
    ...
    saveAs(base64Image, `Certificado_NotasFacultad${gestion}.png`); // Guardar imagen como archivo PNG
  };
  ...
  return (
    <Row>
      <Col xs={12} md={5} xl={5}>
        <Card>
          <CardHeader>
            Certificado de Notas por Sexo, según Facultad.
            <Form.Select
              className="mt-2"
              value={gestion}
