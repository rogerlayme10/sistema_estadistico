...
const BecaPrincipal = () => {
    ...
    useEffect(() => {
        fetch(`${config.API_URL}/movilidad-principal`)
            .then(response => response.json())
            .then(data => {
                const gestionLabels = data.map(item => item.gestion);
                const docentesCounts = data.map(item => item.total_tramites);

                setChartData({
                    labels: gestionLabels,
                    datasets: [
                        {
                            label: 'Cronologia del Movimineto estduiantil',
                            data: docentesCounts,
    ...
    return (
        <Row>
            <Col xs={18} md={9} xl={9}>
                <Card>
                    <CardHeader>Cronologia del movimiento estudiantil.</CardHeader>
                    <Card.Body>
