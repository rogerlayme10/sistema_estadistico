...
const DocentesCrecimiento = () => {
    ...
    useEffect(() => {
        fetch(`${config.API_URL}/docentes-gestiongrafic`)
            .then(response => response.json())
            .then(data => {
                const gestionLabels = data.map(item => item.gestion);
                const docentesCounts = data.map(item => item.total_docentes);
    ...
    const downloadChart = () => {
        ...
        link.download = 'crecimiento-docentes.png';
        link.click();
    };
    ...
    return (
        <Row>
            <Col xs={18} md={9} xl={9}>
                <Card>
                    <CardHeader>Cronología del Personal Docente por gestión.</CardHeader>
                    <Card.Body>
