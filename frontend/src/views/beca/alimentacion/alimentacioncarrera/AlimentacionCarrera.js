...
const AlimentacionCarrera = () => {
    ...
    const fetchData = async (year = null) => {
        try {
            const url = year ? `${config.API_URL}/alimentacion-carrera/${year}` : `${config.API_URL}/alimentacion-carrera`;
    ...
    const exportToExcel = () => {
        ...
        XLSX.writeFile(wb, `becas_alimentacion_carrera${gestion}.xlsx`);
    };
    ...
    return (
        <Row>
            <Col >
                <Card>
                    <CardHeader>
                        Beca Alimentación por Sexo, según Carrera.
                        <Form.Select 
                            className="mt-2" 
                            value={gestion} 
...
export default AlimentacionCarrera;
