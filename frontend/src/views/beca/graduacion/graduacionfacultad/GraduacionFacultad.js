...
const GraduacionFacultad = () => {
  ...
  const fetchData = async (year = null) => {
    try {
      const url = year ? `${config.API_URL}/graduacion-facultad/${year}` : `${config.API_URL}/graduacion-facultad`;
  ...
  return (
    <Row>
      <Col xs={12} md={5} xl={5}>
        <Card>
          <CardHeader>
            Beca Graduación por sexo, según Facultad.
            <Form.Select
              className="mt-2"
              value={gestion}
...
export default GraduacionFacultad;
