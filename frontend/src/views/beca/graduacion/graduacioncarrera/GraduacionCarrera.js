...
const GraduacionCarrera = () => {
    ...
    const fetchData = async (year = null) => {
        try {
            const url = year ? `${config.API_URL}/graduacion-carrera/${year}` : `${config.API_URL}/graduacion-carrera`;
    ...
    return (
        <Row>
            <Col >
                <Card>
                    <CardHeader>
                        Beca Graduación por sexo, según Carrera.
                        <Form.Select 
                            className="mt-2" 
                            value={gestion} 
...
export default GraduacionCarrera;
