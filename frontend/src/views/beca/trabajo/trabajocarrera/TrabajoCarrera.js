...
const TrabajoCarrera = () => {
    ...
    const fetchData = async (year = null) => {
        try {
            const url = year ? `${config.API_URL}/trabajo-carrera/${year}` : `${config.API_URL}/trabajo-carrera`;
    ...
    const exportToExcel = () => {
        ...
        XLSX.writeFile(wb, `beca_Trabajo_Carrera${gestion}.xlsx`);
    };
    ...
    return (
        <Row>
            <Col >
                <Card>
                    <CardHeader>
                        Beca Trabajo por Sexo, según Carrera.
                        <Form.Select 
                            className="mt-2" 
                            value={gestion} 
