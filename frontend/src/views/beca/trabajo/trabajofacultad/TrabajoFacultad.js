...
const TrabajoFacultad = () => {
  ...
  const fetchData = async (year = null) => {
    try {
      const url = year ? `${config.API_URL}/trabajo-facultad/${year}` : `${config.API_URL}/trabajo-facultad`;
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `Beca_TrabajoFacultad${gestion}.xlsx`);
  };
  ...
  const downloadChartImage = () => {
    ...
    saveAs(base64Image, `Beca_TrabajoFacultad${gestion}.png`); // Guardar imagen como archivo PNG
  };
  ...
  return (
    <Row>
      <Col xs={12} md={5} xl={5}>
        <Card>
          <CardHeader>
          Beca Trabajo por Sexo, según Facultad.
            <Form.Select
              className="mt-2"
              value={gestion}
...
export default TrabajoFacultad;
