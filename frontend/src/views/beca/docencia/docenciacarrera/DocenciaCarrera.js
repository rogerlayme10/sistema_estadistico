...
const DocenciaCarrera = () => {
    ...
    const fetchData = async (year = null) => {
        try {
            const url = year ? `${config.API_URL}/docencia-carrera/${year}` : `${config.API_URL}/docencia-carrera`;
    ...
    const exportToExcel = () => {
        ...
        XLSX.writeFile(wb, `Becas_AuxDocenciaCarrera${gestion}.xlsx`);
    };
    ...
    return (
        <Row>
            <Col >
                <Card>
                    <CardHeader>
                        Beca Auxiliar de Docencia por Sexo, según Carrera.
                        <Form.Select 
                            className="mt-2" 
                            value={gestion} 
...
export default DocenciaCarrera;