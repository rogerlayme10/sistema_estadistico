...
const InvestigacionFacultad = () => {
  ...
  const fetchData = async (year = null) => {
    try {
      const url = year ? `${config.API_URL}/investigacion-facultad/${year}` : `${config.API_URL}/investigacion-facultad`;
  ...
  return (
    <Row>
      <Col xs={12} md={5} xl={5}>
        <Card>
          <CardHeader>
            Beca investigación por Sexo, según Facultad.
            <Form.Select
              className="mt-2"
              value={gestion}
...
export default InvestigacionFacultad;
