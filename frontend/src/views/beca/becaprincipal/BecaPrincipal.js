...
const BecaPrincipal = () => {
    ...
    useEffect(() => {
        fetch(`${config.API_URL}/beca-principal`)
            .then(response => response.json())
            .then(data => {
                const gestionLabels = data.map(item => item.gestion);
                const docentesCounts = data.map(item => item.total_becas);

                setChartData({
                    labels: gestionLabels,
                    datasets: [
                        {
                            label: 'Cronologia del beca alimentacion',
                            data: docentesCounts,
    ...
    return (
        <Row>
            <Col xs={18} md={9} xl={9}>
                <Card>
                    <CardHeader>Cronologia de Becas.</CardHeader>
                    <Card.Body>
