...
class MovilidadCambioResumenController extends Controller
{
    public function getCambioResumen(Request $request, $gestion = null)
    {
        if ($gestion) {
            ...
            $gestiones = DB::table('tramites')
                ...
                ->where(DB::raw("TRIM(tramite)"), 'CAMBIO_CARRERA') // Filtro adicional por tipo_beca = 'ALIMENTACION'
                ->groupBy('gestion')
                ->orderBy('gestion', 'asc')
                ->get();
