...
class MovilidadReadmisionResumenController extends Controller
{
    public function getReadmisionResumen(Request $request, $gestion = null)
    {
        if ($gestion) {
            ...
            $gestiones = DB::table('tramites')
                ...
                ->where(DB::raw("TRIM(tramite)"), 'READMISION') // Filtro adicional por tipo_beca = 'ALIMENTACION'
                ->groupBy('gestion')
                ->orderBy('gestion', 'asc')
                ->get();
