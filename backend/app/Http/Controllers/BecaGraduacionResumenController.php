...
class BecaGraduacionResumenController extends Controller
{
    public function getGraduacionResumen(Request $request, $gestion = null)
    {
        if ($gestion) {
            ...
            $gestiones = DB::table('becas')
                ...
                ->where(DB::raw("TRIM(tipo_beca)"), 'GRADUACION') // Filtro adicional por tipo_beca = 'ALIMENTACION'
                ->groupBy('gestion')
                ->orderBy('gestion', 'asc')
                ->get();
