...
class BecaTrabajoResumenController extends Controller
{
    public function getTrabajoResumen(Request $request, $gestion = null)
    {
        if ($gestion) {
            ...
            $gestiones = DB::table('becas')
                ...
                ->where(DB::raw("TRIM(tipo_beca)"), 'TRABAJO') // Filtro adicional por tipo_beca = 'ALIMENTACION'
                ->groupBy('gestion')
                ->orderBy('gestion', 'asc')
                ->get();
