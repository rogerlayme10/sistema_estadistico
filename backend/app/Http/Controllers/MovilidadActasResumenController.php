...
class MovilidadActasResumenController extends Controller
{
    public function getActasResumen(Request $request, $gestion = null)
    {
        if ($gestion) {
            ...
            $gestiones = DB::table('tramites')
                ...
                ->where(DB::raw("TRIM(tramite)"), 'ACTAS') // Filtro adicional por tipo_beca = 'ALIMENTACION'
                ->groupBy('gestion')
                ->orderBy('gestion', 'asc')
                ->get();
