...
class AdmisionPsaCrecimientoPostulantesController extends Controller
{
    public function getPsaTasaCrecimientoPostulantesCarrera(Request $request)
    {
        ...
        foreach ($gestiones as $gestion) {
            $query->addSelect(
                DB::raw("SUM(CASE WHEN gestion = $gestion AND id_examen = '1' THEN 1 ELSE 0 END) as total_$gestion")
            );
        }
    
        // Agregar la columna total general
        $query->addSelect(DB::raw("COUNT(CASE WHEN id_examen = '1' THEN 1 ELSE NULL END) as total"));
    
        // Ejecutar la consulta
        $result = $query
            ->where('id_examen', '=', '1') // Solo estudiantes psa
            //->where('admisions.estado', '=', 'A') solo postulantes 
