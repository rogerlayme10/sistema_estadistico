...
class MovilidadTransferenciaResumenController extends Controller
{
    public function getTransferenciaResumen(Request $request, $gestion = null)
    {
        if ($gestion) {
            ...
            $gestiones = DB::table('tramites')
                ...
                ->where(DB::raw("TRIM(tramite)"), 'TRANSFERENCIA') // Filtro adicional por tipo_beca = 'ALIMENTACION'
                ->groupBy('gestion')
                ->orderBy('gestion', 'asc')
                ->get();
