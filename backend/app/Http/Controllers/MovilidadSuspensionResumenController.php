...
class MovilidadSuspensionResumenController extends Controller
{
    public function getSuspensionResumen(Request $request, $gestion = null)
    {
        if ($gestion) {
            ...
            $gestiones = DB::table('tramites')
                ...
                ->where(DB::raw("TRIM(tramite)"), 'SUSPENSION') // Filtro adicional por tipo_beca = 'ALIMENTACION'
                ->groupBy('gestion')
                ->orderBy('gestion', 'asc')
                ->get();
