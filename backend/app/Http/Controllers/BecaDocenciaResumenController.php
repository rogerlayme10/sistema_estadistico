...
class BecaDocenciaResumenController extends Controller
{
    public function getDocenciaResumen(Request $request, $gestion = null)
    {
        if ($gestion) {
            ...
            $gestiones = DB::table('becas')
                ...
                ->where(DB::raw("TRIM(tipo_beca)"), 'AUXILIAR DE DOCENCIA') // Filtro adicional por tipo_beca = 'ALIMENTACION'
                ->groupBy('gestion')
                ->orderBy('gestion', 'asc')
                ->get();
