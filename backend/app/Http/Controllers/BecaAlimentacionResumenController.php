...
class BecaAlimentacionResumenController extends Controller
{
    public function getAlimentacionResumen(Request $request, $gestion = null)
    {
        if ($gestion) {
            ...
            $gestiones = DB::table('becas')
                ...
                ->where(DB::raw("TRIM(tipo_beca)"), 'ALIMENTACION') // Filtro adicional por tipo_beca = 'ALIMENTACION'
                ->groupBy('gestion')
                ->orderBy('gestion', 'asc')
                ->get();
